...
public class CacheMonitorService {
    ...
    public Map<String, Object> getCacheStatistics() {
        ...
        if (accountRepository instanceof RepositoryProxy) {
            RepositoryProxy.CacheStats accountStats =
                    ((RepositoryProxy<BankAccount>) accountRepository).getCacheStats();
            stats.put("accounts", accountStats);
        }

        if (categoryRepository instanceof RepositoryProxy) {
            RepositoryProxy.CacheStats categoryStats =
                    ((RepositoryProxy<Category>) categoryRepository).getCacheStats();
            stats.put("categories", categoryStats);
        }

        if (operationRepository instanceof RepositoryProxy) {
            RepositoryProxy.CacheStats operationStats =
                    ((RepositoryProxy<Operation>) operationRepository).getCacheStats();
